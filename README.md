...
从旧版升级请先阅读 [`数据迁移教程.txt`](数据迁移教程.txt)；版本变化统一记录在 [`更新日志.md`](更新日志.md)。

![Afterhours 2.0.2 烟粉主页与极地雾统计页](afterhours-2.0.1-preview.png)
...
- **可调整窗口**：可拖动右下角或窗口边缘改变大小，最窄可缩至 300 像素；字体、按钮、热力格、卡片和统计图会随窗口等比缩放，热力图卡片减少两侧留白，纵向空间不足时可滚动；折叠后点击“+”会恢复折叠前的自定义尺寸。
- **主题与深色模式**：提供极地雾、极光薄荷、烟粉、暮光紫四种主题，主题同步作用于热力图、标签、进度条和统计图表。
...
2.0.2 不提供长期数据导入按钮。若要保留 1.0 历史，请在第一次启动 2.0.2 前阅读包内的 `数据迁移教程.txt`，关闭两个版本后复制指定数据文件。1.0 没有保存具体会话起止时间，所以历史每日累计可以保留，时间轴只能从 2.0 开始记录。
